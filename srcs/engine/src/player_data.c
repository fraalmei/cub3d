...
void	player_data(t_game *game)
{
	...
	pos_x = -1;
	while (++pos_x < game->map->map_size.x)
	{
		pos_y = -1;
		while (++pos_y < game->map->map_size.y)
			if (game->matrix[pos_x][pos_y] != '1')
				player_data_aux(game, pos_x, pos_y, game->matrix[pos_x][pos_y]);
	}
	//PRINT_DEBUG("Funcion [%s]: Final | Pos[%f][%f] | FOV_RD [%0.7f] | Angle [%0.7f]", __func__, game->player_init_pos.x, game->player_init_pos.y, game->player.fov_rd, game->player.angle);
}
